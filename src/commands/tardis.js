...
async function applications (params) {
  ...
  const applications = await ApiHelpers.getWarp10Applications({ id: ownerId }).then(sendToApi);
...
async function applicationTokens (params) {
  ...
  const tokens = await ApiHelpers.getAllTokens({ id: ownerId, app: warp10Application }).then(sendToApi);
...
async function createReadToken (params) {
  const [ warp10Application ] = params.args;
  const { org: orgaIdOrName, ttl: tardisTTL, properties: properties } = params.options;
  const ownerId = await Organisation.getId(orgaIdOrName);
...
async function createWriteToken (params) {
  const [ warp10Application ] = params.args;
  const { org: orgaIdOrName, ttl: tardisTTL } = params.options;
  const ownerId = await Organisation.getId(orgaIdOrName);
