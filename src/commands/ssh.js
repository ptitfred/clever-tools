...
async function ssh (params) {
  const { alias, app: appIdOrName, org: orgIdOrName, 'identity-file': identityFile } = params.options;

  const { appId } = await Application.resolveId(appIdOrName, orgIdOrName, alias);
